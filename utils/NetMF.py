...
class NetMF(Estimator):
    ...
    def __init__(self, dimensions: int = 32, iteration: int = 10, order: int = 2, negative_samples: int = 1,
                 seed: int = 42):
        super().__init__()
    ...
    def _create_D_inverse(self, graph):
        ...
        values = np.array(
            [1.0 / (graph.degree[node] + self.lower_control) for node in range(graph.number_of_nodes())]
        )
